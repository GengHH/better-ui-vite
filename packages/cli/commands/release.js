...
const packageJson = require('../../hhui/package.json');
...
exports.release = async ({ version }) => {
	...
	shell.cp('-R', path.resolve(__dirname, '../../hhui/README.md'), outputDir);
	shell.cd(outputDir);
	shell.mkdir('-p', 'theme');
	shell.cp(
		'-R',
		path.resolve(__dirname, '../../hhui/ui/theme/theme.scss'),
		path.resolve(outputDir, 'theme')
	);
	shell.cp(
		'-R',
		path.resolve(__dirname, '../../hhui/ui/theme/darkTheme.css'),
		path.resolve(outputDir, 'theme')
	);
